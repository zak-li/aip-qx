...
export default function OptionsPanel({ open }) {
  ...
  function exportConv() {
    ...
    if (!msgs.length) { toast('NO CONVERSATION', 'warn'); return; }
    const lines = ['# HET-X RAG — Export', `> ${new Date().toLocaleString('fr-FR')}`, '---', ''];
    msgs.forEach(m => {
      lines.push(m.role === 'user' ? `**[OPERATOR]**\n\n${m.content}` : `**[HET-X]**\n\n${m.content}`);
      lines.push('\n---\n');
    });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([lines.join('\n')], { type: 'text/markdown' }));
    a.download = `hetx-${Date.now()}.md`;
    a.click();
    toast('EXPORTED', 'ok');
  }
    ...
    <div className={`opts-panel${open ? ' open' : ''}`}>
      <div className="opts-body">
        ...
        <Slider label="Température"  value={opts.temperature} min={0} max={1} step={0.05}
          display={opts.temperature.toFixed(2)} hint="Bas = précis · Haut = créatif"
          onChange={v => opts.set({ temperature: v })} />

        <Slider label="Top-P" value={opts.topP} min={0} max={1} step={0.05}
          display={opts.topP.toFixed(2)} hint="Nucleus sampling"
        ...
        <div className="opt-group">
          ...
          <span className="opt-hint">Messages d'historique</span>
        </div>
        ...
        <div className="opt-group opts-full-row">
          ...
          <span className="opt-hint">Injecte une directive de format dans le system prompt</span>
        </div>

        <hr className="opts-section-sep" />

        <div className="opt-group opt-actions opts-full-row">
          <button className="opt-act danger" onClick={() => { clear(); toast('CONTEXT CLEARED', 'ok'); }}>
            Clear context
          </button>
          <button className="opt-act" onClick={exportConv}>
            Export MD
          </button>
        </div>
