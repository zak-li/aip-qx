...
There exists an objective ground truth $\omega \in \{\text{FRAUD}, \text{LEGITIMATE}\}$.

Each auditor $i \in N$ observes evidence $E$ and must submit a vote $v_i \in \{\text{FRAUD}, \text{LEGITIMATE}\}$ using a cryptographic Commit-Reveal scheme to prevent information cascades (free-riding).
...
If auditor $i$ deviates and plays $\bar{v} \neq \omega$ (a malicious or lazy vote), the supermajority $D = \omega$ is still reached (assuming $n \geq 4$ and $\tau = 2/3$). The deviating auditor is slashed:
\[ u_i(\bar{v}, \omega_{-i}) = S - f \cdot S - C' \]
(where $C'$ is $0$ if they were lazy and did not analyze $E$, or $C$ if they did).
...
\end{lemma}
\begin{proof}
Because $S$ represents institutional operational licenses (not merely liquid capital), $S \to \infty$ from the perspective of an individual rogue trader. The minimum bribe required to compensate auditor $i$ for risking their stake is $f \cdot S_i$; summing over a coalition of size $k$ yields $B_{cost} = k \cdot f \cdot S$. Therefore, the total cost of bribing $k$ institutions to risk their regulatory banking licenses grows without bound, rendering any such coalition economically infeasible.
\end{proof}
